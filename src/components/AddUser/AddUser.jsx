...
function AddUser({ currentMembers, userId, creatorId, projectId }) {
            ...
            options={userList?.map(member => {
                return {
                    ...
                    label: <button className='flex items-center' onClick={() => {
                        ...
                        <Avatar className='mr-2' title={member.name} src={member.avatar} key={member.userId} />
    ...
    return (
        <Popover placement="bottomRight" title={title} content={content} trigger="click" >
            <Button className='bg-blue-600 text-white flex justify-center items-center' shape="circle" icon={<PlusOutlined />} title='Add members' onClick={() => {
                if (userId !== creatorId) {
                    Swal.fire({
                        icon: 'error',
